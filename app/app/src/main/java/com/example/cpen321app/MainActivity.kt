...
//        }
    ...
    private fun signInSuccess(googleCredential: GoogleIdTokenCredential) {
        ...
        // Acquire location permissions for notifications
        ...
        var finePermissionGranted = ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED
        var coarsePermissionGranted = ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED

        Log.d(TAG, "shouldShowRequestPermissionRationale for ACCESS_FINE_LOCAITON: $shouldShowFineRationale")
        Log.d(TAG, "shouldShowRequestPermissionRationale for ACCESS_COARSE_LOCAITON: $shouldShowCoarseRationale")
        Log.d(TAG, "Permissions granted: ACCESS_FINE_LOCATION = $finePermissionGranted, ACCESS_COARSE_LOCATION = $coarsePermissionGranted")

        checkLocationPermissions(finePermissionGranted, coarsePermissionGranted, shouldShowFineRationale, shouldShowCoarseRationale)

//        startWorkerIfPermsGranted() Not so sure if this function is correct to use. It doesn't work anyways. It also runs before location permissions get granted. Need to delay it somehow.
    ...
    private fun startWorkerIfPermsGranted() {
        ...
        if(finePermissionGranted && coarsePermissionGranted) {
            val locationWorkRequest = PeriodicWorkRequestBuilder<LocationWorker>(30, TimeUnit.SECONDS).build()
            WorkManager.getInstance(applicationContext).enqueue(locationWorkRequest)
        }
    }

    private fun checkLocationPermissions(finePermissionGranted: Boolean, coarsePermissionGranted: Boolean, shouldShowFineRationale: Boolean, shouldShowCoarseRationale: Boolean) {
        when {
            finePermissionGranted && coarsePermissionGranted -> {
    ...
    private fun showLocationPermissionRationale() {
        AlertDialog.Builder(this).setMessage("Location permission is required for task notifications.")
