...
class AddTask : AppCompatActivity() {
    ...
    private fun isTimeAfter(earlierTime: String, laterTime: String): Boolean {
        val formatter = DateTimeFormatter.ofPattern("HH:mm") // Define expected time format
        val firstTime = LocalTime.parse(laterTime, formatter)
        val secondTime = LocalTime.parse(earlierTime, formatter)

        return firstTime.isAfter(secondTime) // Returns true if time1 > time2
    }
}
